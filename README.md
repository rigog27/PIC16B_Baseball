This project builds a recurrent neural network that predicts the next pitch type thrown by MLB pitcher 
Yoshinobu Yamamoto, who uses a diverse six-pitch mix:
...
Pitch prediction is framed as a sequence classification problem, where each plate appearance (PA)
is treated as a variable-length sequence of pitches. At every timestep, the model uses player-specific
and contextual statistics to identify the most likely next pitch.
